...
def work(cost_pertime, time, freq, df_300, df_nas, bCut, up, sell_rate):
    ...
    for i in range(times):
        ...
        if j == 0:
            ...
            #如果进行过止盈止损，把卖出得到的钱加上
                ...
                if bUp300 == True and rate300[i-1] - minRate300 > stop*up:
            ...
            if bCutUpNas == False:
                ...
                if bUpNas == True and rateNas[i-1] - minRateNas > stop*up:
        ...
        #print(i, cost300[i], value300[i], rate300[i])
        ...
        if bCut == True:
            ...
            if i != 0:
                ...
                if bCutUp300 == True:
                    ...
                    #print(i, cost300[i], money_cut_300, Cut300)
                    cost300[i] += Cut300[2]
                    money_cut_300 += Cut300[1] - Cut300[2]
                    rate300[i] = (value300[i] - cost300[i] + money_cut_300)/cost300[i]
                    fee[i] += Cut300[2]
                    bUp300 = True
                    maxRate300 -= up
                    #print(i, cost300[i], money_cut_300, Cut300)
                    print(i, value300[i], cost300[i], rate300[i])
                ...
                if bCutUpNas == True:
                    ...
                    rateNas[i] = (valueNas[i] - costNas[i] - money_cut_nas)/costNas[i]
                    bUpNas = True
                    maxRateNas -= up
                    fee[i] += CutNas[2]
                    #print(i, "纳指")
    ...
    #单独画两个个股的收益率看看。
    fig = plt.figure()
    plt.plot(rate300, label="300_test")
    plt.plot(df_300["close"]/df_300["close"][1]-1, label = "300etf")
    plt.legend(loc = "best")
    fig.savefig("300etf_test.png")
    
    fig = plt.figure()
    plt.plot(rateNas, label="Nas_test")
    plt.plot(df_nas["close"]/df_nas["close"][1]-1, label = "Nasetf")
    plt.legend(loc = "best")
    fig.savefig("Nasetf_test.png")
...
def Cut(stack, price, fee_rate, sell_rate):
    ...
    #print(stack, sellNumber, price, sellMoney)
...
#    etfnas.to_csv("df_nas_hist.csv")
    ...
    freq = 10
    times = len(df_300)
    data = work(1000, times, freq, df_300, df_nas, False, 0.15, 0.5)
    #计算指标
    #先处理基准指标，剔除没交易的日期的数据
    ...
    #画图看看吧。
    fig = plt.figure()
    plt.plot(testdata[("数据")], label = "Total")
    plt.plot(df_300["close"]/df_300["close"][1]-1, label = "300etf")
    plt.plot(df_nas["close"]/df_nas["close"][1]-1, label = "Nasetf")
    plt.legend(loc = "best")
